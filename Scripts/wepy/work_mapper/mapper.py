...
class Mapper(ABCMapper):
    ...
    def map(self,*args, **kwargs):
        ...
        *args : list of list
        ...
        dcd_folder = args[0]
        save_freq = args[1]
        args = [list(arg) for arg in args[2:]]
        kwargs = {key: list(kwarg) if kwarg is not None else [] for key, kwarg in kwargs.items()}
        ...
        for arg_idx in range(len(args[0])):
            ...
            try:
                ...
                result = self._func(dcd_folder = dcd_folder,save_freq=save_freq,*call_args, **call_kwargs)
