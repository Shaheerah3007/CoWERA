...
class WepyHDF5(object):
    ...
    def _run_record_namedtuple(self, run_record_key):
        ...
        Ensures that field names are valid Python identifiers (decode bytes,
        replace invalid chars, handle leading digits).
        """
        raw_fields = self.record_fields[run_record_key]

        clean_fields = []
        for f in raw_fields:
            if isinstance(f, bytes):
                f = f.decode()
            # replace illegal chars with underscore and prefix if starts with digit
            f = re.sub(r'\W|^(?=\d)', '_', f)
            clean_fields.append(f)

        Record = namedtuple(f"{run_record_key}_Record", [CYCLE_IDX] + clean_fields)
        return Record
    ...
    def resampling_records_dataframe(self, run_idxs):
        ...
        records = self.resampling_records(run_idxs)
        if hasattr(records, 'dtype') and records.dtype.names:
            records.dtype.names = tuple(
