...
class OpenMMRunner(Runner):
    ...
    def run_segment(self,
        ...
        if os.path.exists(dcd_path):
            dcd_reporter = omma.DCDReporter(dcd_path, self.save_freq, append=True)
        else:
            dcd_reporter = omma.DCDReporter(dcd_path, self.save_freq)
        simulation.reporters = [dcd_reporter]
