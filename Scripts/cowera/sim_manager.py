...
class Manager(object):
    ...
    def __init__(self, init_walkers,
        ...
        if outputs_dir is not None:
            logging.basicConfig(
                filename=f'{self.outputs_dir}/dashboard.log',
                level=logging.INFO,
    ...
    def _run_cycle(self,
        ...
        resampling_results= self.resampler.resample(warped_walkers,cycle_idx,self.n_bins, self.max_bins)

        end = time.time()
        resampling_time = end - start

        resampled_walkers = resampling_results[0]
        #print(np.array([walker.weight for walker in walkers]).sum())
        resampling_data = resampling_results[1]
        resampler_data = resampling_results[2]
        self.n_bins = resampling_results[3]
